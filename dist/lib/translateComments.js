...
export async function translateComments(javaCode, comments, fromLanguage = 'English', toLanguage = 'Japanese', maxIteration = 3) {
...
<java-source-file-content>${javaCode}</java-source-file-content>          
          `,
                },
                {
                    role: 'user',
                    content: comments
                        .map((comment, idx) => `<selected-comment index:"${idx}">${comment}</selected-comment>`)
                        .join('\n'),
                },
            ];
