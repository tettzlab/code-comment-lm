...
import { parse, IToken, CstNode } from 'java-parser'
import { JavaCommentVisitor, LINE_COMMENT, TRADITIONAL_COMMENT } from './JavaCommentVisitor.js'
import { JavaComment, JavaCommentToken } from './JavaComment.js'
import { identifyLineSeparator } from './identifyLineSeparator.js'
import { translateComments } from '../lib/translateComments.js'

export interface JavaCommentTranslatorOptions {
  from: string // default: English
  to: string // default: Japanese
}

export class JavaCommentTranslator {
  public code: string[] | undefined
  public comments: JavaCommentToken[] | undefined
  public cst: CstNode | undefined
  public lineSeparator: string | undefined
  public mergedComments: JavaComment[] | undefined
  public normalizedCode: string | undefined
  public readonly options: JavaCommentTranslatorOptions = {
    from: 'English',
    to: 'Japanese',
  }
  public toLanguage: string | undefined

  public constructor(options?: Partial<JavaCommentTranslatorOptions>) {
    this.options = { ...this.options, ...(options ?? {}) }
  }

  public async translate(
    javaCode: string,
    options?: Partial<JavaCommentTranslatorOptions>,
  ): Promise<string> {
  ...
  private enhanceTokens(tokens: IToken[]): JavaCommentToken[] {
    const javaCodeLine: string[] = this.code as string[]
    const comments: JavaCommentToken[] = []
    let i = 0
    ...
    while (i < len) {
      ...
      const comment: JavaCommentToken = {
        ...token,
        lines,
        isLineComment,
  ...
  private mergeTokens(tokens: JavaCommentToken[]): JavaComment[] {
    const merged: JavaComment[] = []
    let i = 0
    const len = tokens.length
    while (i < len) {
      const conseq: JavaCommentToken[] = []
      let head = tokens[i++]
      conseq.push(head)
